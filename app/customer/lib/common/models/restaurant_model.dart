...
import '../../features/wallet/domain/models/fund_bonus_model.dart';
...
class Restaurant {
  ...
  String get fullCover {
    return coverPhoto!.contains('http')
        ? coverPhoto!
        : "${getx.Get.find<SplashController>().configModel!.baseUrls!.restaurantCoverPhotoUrl}/$coverPhoto";
  }
  ...
  bool operator ==(covariant Restaurant other) {
    ...
    return other.name?.justCharacters == name?.justCharacters;
  }

  @override
  int get hashCode {
    return name?.justCharacters.hashCode ?? 0;
  }

  // distance is in km, convert to miles
  double get distanceInMile {
    return (distance ?? 0) * 0.621371;
  }

  String get deliveryTimeFormatted {
    final time = '$deliveryTime'.split('-');
    return '${time[0]} - ${time[1]} ${time[2].capitalize}';
  }

  double get deliveryAmount {
    return (freeDeliveryDistanceStatus ?? false)
        ? (freeDeliveryDistanceValue ?? 0)
        : (perKmShippingCharge ?? 0) * distanceInMile;
  }
}
