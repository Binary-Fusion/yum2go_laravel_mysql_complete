...
class ProductService implements ProductServiceInterface {
  ...
  @override
  List<List<bool?>> setCartVariationEnabledIndex(
      int index,
      int i,
  ...
  List<List<bool?>> setCartVariationIndex(
    ...
    if (defaultValue != null) {
      if (isMultiSelect) {
        resultVariations[index][i] = defaultValue;
        return resultVariations;
      } else {
        for (int j = 0; j < resultVariations[index].length; j++) {
          if (j == i) {
            resultVariations[index][j] = defaultValue;
          } else {
            resultVariations[index][j] = false;
          }
        }
