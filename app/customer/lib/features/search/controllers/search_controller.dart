    ...
    if (_searchSuggestionModel != null) {
      for (var food in _searchSuggestionModel!.foods!) {
        if (!foods
            ...
            .contains(food.name!.trim().toLowerCase())) {
          foods.add(food.name!);
        }
      }
      for (var restaurant in _searchSuggestionModel!.restaurants!) {
        if (!foods
            .map((e) => e.trim().toLowerCase())
            .contains(restaurant.name!.trim().toLowerCase())) {
      ...
      if (response.statusCode == 200) {
        if (query.isEmpty) {
          ...
          if (_isRestaurant) {
            ...
            _searchRestList = _searchRestList!.toSet().toList();
            _allRestList = _allRestList!.toSet().toList();
          } else {
            ...
            _allProductList!
                .addAll(ProductModel.fromJson(response.body).products ?? []);
            _searchProductList = _searchProductList!.toSet().toList();
            _allProductList = _allProductList!.toSet().toList();
          }
        }
      }
      update();
