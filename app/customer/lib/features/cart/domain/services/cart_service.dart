...
class CartService implements CartServiceInterface {
  ...
  @override
  Future<List<CartModel>> formatOnlineCartToLocalCart(
      {required List<OnlineCartModel> onlineCartModel}) {
    return CartHelper.formatOnlineCartToLocalCart(
        onlineCartModel: onlineCartModel);
    ...
    //   for (int index = 0; index < cart.product!.allVariations.length; index++) {
    //     selectedFoodVariations.add([]);
    //     collapsVariation.add(true);
    //     variationsStock.add([]);
    //     for (int i = 0;
    //         i < cart.product!.allVariations[index].variationValues!.length;
    //         i++) {
    //       Get.log(
    //           'checking ${cart.product!.allVariations[index].name} with ${cartVariationsNameList}');
    //       variationsStock[index].add(cart
    //           .product!.allVariations[index].variationValues![i].currentStock);
    //       if (cartVariationsNameList
    //               .contains(cart.product!.allVariations[index].name) &&
    //           cart.variation!
    //               .where(
    //                   (e) => e.name == cart.product!.allVariations[index].name)
    //               .first
    //               .values!
    //               .label!
    //               .contains(cart.product!.allVariations[index]
    //                   .variationValues![i].level)) {
    //         Get.log(
    //             '${cart.product!.allVariations[index].name} ${cart.variation!.where((e) => e.name == cart.product!.allVariations[index].name).first.values!.label!} ${cart.product!.allVariations[index].variationValues![i].level} - true');
    //         selectedFoodVariations[index].add(true);
    //       } else {
    //         Get.log(
    //             '${cart.product!.allVariations[index].name} ${cart.variation!.where((e) => e.name == cart.product!.allVariations[index].name).first.values!.label!} ${cart.product!.allVariations[index].variationValues![i].level} - false');
  ...
  double calculateVariationWithoutDiscountPrice(CartModel cartModel,
    ...
    if (cartModel.product!.allVariations.isNotEmpty) {
      for (int index = 0;
          index < cartModel.product!.allVariations.length;
          index++) {
        for (int i = 0;
            i < cartModel.product!.allVariations[index].variationValues!.length;
            i++) {
          if (cartModel.variations![index][i]!) {
            variationWithoutDiscountPrice +=
                (PriceConverter.convertWithDiscount(
                        cartModel.product!.allVariations[index]
    ...
    if (cartModel.product!.allVariations.isNotEmpty) {
      for (int index = 0;
        ...
        for (int i = 0;
            ...
            i++) {
          if (cartModel.variations![index][i]!) {
            variationPrice += (cartModel.product!.allVariations[index]
                    .variationValues![i].optionPrice! *
                cartModel.quantity!);
          }
        }
