import 'package:yum2go_v3_customer/api/api_checker.dart';
...
import 'package:yum2go_v3_customer/common/widgets/cart_snackbar_widget.dart';
...
class CartController extends GetxController implements GetxService {
  ...
  List<String> _variationText = [];
  List<String> get variationText {
    if (_cartList.length != _variationText.length) {
      return List.filled(_cartList.length, '');
    }
  ...
  Future<void> getVariationText() async {
    List<String> tempList = [];
    for (var cartModel in _cartList) {
      tempList.add(await CartHelper.setupVariationText(cart: cartModel));
    }
    _variationText = tempList;
  }
  ...
  Future<void> addToCartOnline(OnlineCart onlineCart,
    ...
    if (response.statusCode == 200) {
      ...
      print('onlineCartList: ${onlineCartList.map((e) => e.toJson())}');
      _cartList = [];
      _cartList.addAll(await cartServiceInterface.formatOnlineCartToLocalCart(
          onlineCartModel: onlineCartList));
      _variationText = [];
      await getVariationText();
      calculationCart();
      if (!fromDirectlyAdd) {
        Get.back();
      }
  ...
  Future<void> getCartDataOnline() async {
    ...
    _cartList.addAll(await cartServiceInterface.formatOnlineCartToLocalCart(
        onlineCartModel: onlineCartList));
    _variationText = List.filled(_cartList.length, '');
    await getVariationText();
    calculationCart();
    _isLoading = false;
    update();
