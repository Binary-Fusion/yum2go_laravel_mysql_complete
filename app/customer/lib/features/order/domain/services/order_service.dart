...
import 'package:yum2go_v3_customer/features/order/domain/models/delivery_log_model.dart';
import 'package:yum2go_v3_customer/features/order/domain/models/order_cancellation_body.dart';
...
import 'package:yum2go_v3_customer/features/order/domain/models/pause_log_model.dart';
import 'package:yum2go_v3_customer/features/order/domain/models/subscription_schedule_model.dart';
...
class OrderService implements OrderServiceInterface {
  ...
  dynamic _sortOutProductAddToCard(List<Variation>? orderedVariation,
    ...
    if (currentFood.variations.isNotEmpty) {
  ...
  Future<bool> checkProductVariationHasChanged(List<CartModel> cartList) async {
    ...
    for (CartModel cart in cartList) {
      if (cart.product!.allVariations.isNotEmpty) {
        for (var pv in cart.product!.allVariations) {
          int selectedValues = 0;

          if (pv.required!) {
            for (var selected
                in cart.variations![cart.product!.allVariations.indexOf(pv)]) {
              if (selected!) {
