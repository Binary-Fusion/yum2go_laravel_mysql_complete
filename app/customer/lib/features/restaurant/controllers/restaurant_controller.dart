...
import 'package:yum2go_v3_customer/features/restaurant/domain/models/cart_suggested_item_model.dart';
...
import 'package:yum2go_v3_customer/helper/extension/num_ext.dart';
...
class RestaurantController extends GetxController implements GetxService {
  ...
  List<Restaurant>? _nationalRestaurantList;
  List<Restaurant>? get nationalRestaurantList => _nationalRestaurantList;

  List<Restaurant>? _localRestaurantList;
  List<Restaurant>? get localRestaurantList => _localRestaurantList;
  ...
  int get totalCount => (_restaurantModel!.totalSize ?? 0) + (_localRestaurantList?.length ?? 0) + (_nationalRestaurantList?.length ?? 0);
  ...
  Future<void> getRestaurantList(int offset, bool reload,
    ...
    if (restaurantModel != null) {
      ...
      _restaurantModel!.restaurants =
          _restaurantModel!.restaurants!.toSet().toList();
      update();
    }
  }
  ...
  Future<void> getNationalRestaurantList(
      bool reload, String type, bool notify) async {
    _type = type;
    if (reload) {
      _nationalRestaurantList = null;
    }
    if (notify) {
      update();
    }
    if (_nationalRestaurantList == null || reload) {
      _nationalRestaurantList =
          await restaurantServiceInterface.getNationalRestaurantList(type);
      update();
    }
  }

  Future<void> getLocalRestaurantList(
      bool reload, String type, bool notify) async {
    _type = type;
    if (reload) {
      _localRestaurantList = null;
    }
    if (notify) {
      update();
    }
    if (_localRestaurantList == null || reload) {
      _localRestaurantList =
          await restaurantServiceInterface.getLocalRestaurantList(type);
      update();
    }
  }
