...
import 'package:yum2go_v3_customer/features/checkout/widgets/payment_method_bottom_sheet.dart';
...
class OrderPlaceButton extends StatelessWidget {
  ...
  DateTime _processScheduleStartDate() {
    ...
    if (checkoutController.timeSlots != null ||
      ...
      DateTime startTime = checkoutController.timeSlots!.isNotEmpty
          ? checkoutController
    ...
    if (checkoutController.timeSlots != null ||
      ...
      DateTime date = checkoutController.selectedDateSlot == 0
          ...
          : checkoutController.selectedDateSlot == 1
              ? DateTime.now().add(const Duration(days: 1))
              : checkoutController.selectedCustomDate ?? DateTime.now();
      DateTime endTime = checkoutController.timeSlots!.isNotEmpty
          ? checkoutController
  ...
  List<place_order_model.OnlineCart> _generateOnlineCartList() {
    ...
    for (int index = 0; index < cartList.length; index++) {
      ...
      for (int i = 0; i < cart.product!.allVariations.length; i++) {
        if (cart.variations![i].contains(true)) {
          variations.add(place_order_model.OrderVariation(
              name: cart.product!.allVariations[i].name,
              values: place_order_model.OrderVariationValue(label: [])));
          for (int j = 0;
              j < cart.product!.allVariations[i].variationValues!.length;
              j++) {
            if (cart.variations![i][j]!) {
              variations[variations.length - 1].values!.label!.add(
                  cart.product!.allVariations[i].variationValues![j].level);
              if (cart.product!.allVariations[i].variationValues![j].optionId !=
                  null) {
                optionIds.add(cart
                    .product!.allVariations[i].variationValues![j].optionId);
              }
            }
          }
        }
