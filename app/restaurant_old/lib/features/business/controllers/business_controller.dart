...
class BusinessController extends GetxController implements GetxService {
  ...
  Future<ResponseModel> getRegistrationFeeUrl(String id) async {
    Response response = await businessServiceInterface.registrationFeeUrl(id, digitalPaymentName ?? 'stripe');
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      String redirectUrl = response.body['redirect_link'];
      Get.back();
