...
import 'package:stackfood_multivendor_restaurant/features/auth/screens/risk_discloser_screen.dart';
...
import '../../../business/screens/new_registered_user_payment_screen.dart';
import '../models/risk_discloser_data.dart';
...
class AuthService implements AuthServiceInterface {
  ...
  Future<Response> registerRestaurant(Map<String, String> data, XFile? logo,
    ...
    if (response.statusCode == 200) {
      ...
      Get.off(() => RiskDisclosurePage(restaurantId: restaurantId!));
    }
    return response;
  }

  @override
  Future<void> registerRiskDiscloser(RiskDiscloserData data) async {
    final response = await authRepoInterface.registerRiskDiscloser(data);
    if (response.statusCode == 200) {
      Get.off(
        () => NewRegisteredUserPaymentScreen(
          restaurantId: int.parse(data.restaurantId),
        ),
      );
    }
  }
  ...
  Future<ResponseModel?> manageLogin(
    ...
    if (response.statusCode == 200) {
      if (response.body['type'] != null && response.body['type'] == 'signup_payment_pending') {
        Get.to(NewRegisteredUserPaymentScreen(
            restaurantId: response.body['restaurant_id'],
            paymentId: response.body['pending_payment']['id']));
      } else if (response.body['pending_payment'] != null) {
